...
<!--The following code predicts the carbon content two times: The first time, we do not summarize MCMC draws. The second time we do. Then, we plot the densities for both cases.

```{r, eval=FALSE, echo=FALSE}
# predict C content. Note: We rename the column after the first prediciton to make sure it does not get overwritten
d <- 
  d %>%
  dplyr::select(-carbon_content_1_in_pd) %>% # cleanup from first prediction
  irp_carbon_content_1(do_summary = FALSE) %>%
  dplyr::rename(carbon_content_1_mcmc = "carbon_content_1") %>%
  dplyr::select(-carbon_content_1_in_pd) %>%
  irp_carbon_content_1(do_summary = TRUE)
  
# plot the densities for the implied distributions
p1 <- 
  ggplot(mapping = aes(x = d$carbon_content_1_mcmc[[1]])) +
  geom_histogram()
```
-->
...
d1_C_difference <- d1$carbon_content_1[[1]] - d1$carbon_content_1[[2]]

# Visualize as histogram
ggplot() + 
  geom_histogram(aes(x = d1_C_difference))
```
...
d1 <- 
  d %>%
  irp_carbon_content_1(do_summary = TRUE) %>%
  irp_nitrogen_content_1(do_summary = TRUE) %>%
  irp_hydrogen_content_1(do_summary = TRUE) %>%
  irp_oxygen_content_1(do_summary = TRUE) %>%
  irp_phosphorus_content_1(do_summary = TRUE) %>%
  irp_potassium_content_1(do_summary = TRUE)
...
d2 <- 
  d %>%
  irp_predict(variable = target_variables, do_summary = TRUE)


# Compare
identical(d1, d2) #---todo: check whether this works when seeds are set for rbeta, rnorm, rdirichlet!
```
...
d3 <- 
  d %>%
  irp_predict(variable = "all", do_summary = TRUE)
...
a <- 
  irpeat_sample_data %>%
  irp_predict(variable = "bulk_density_1", do_summary = TRUE) %>%
  irp_predict(variable = "macroporosity_1", do_summary = TRUE) %>%
  irp_predict(variable = c("carbon_content_1", "nitrogen_content_1", "hydrogen_content_1", "oxygen_content_1"), do_summary = TRUE)
...
a <- 
  irpeat_sample_data %>%
  irp_predict(variable = "hydrogen_content_1", do_summary = TRUE) %>%
