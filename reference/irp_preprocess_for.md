...
``` r
irp_preprocess_for(x, y = NULL, variable)
```
...
`x` where the spectra now a are the preprocessed spectra which would be
used in the next step to predict the peat property indicated by
`variable`.
