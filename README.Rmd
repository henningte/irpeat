...
3. Holocellulose lignin mass fraction (following @Hodgkins.2018).
...
`x` is identical to `ir::ir_sample_data[1:5, ]`, but contains additional columns for the computed humification indices (h1, h2, h3, h4) and the computed Klason lignin content (klason_lignin_hodgkins) and holocellulose content (holocellulose_hodgkins).
```{r x_inspect}
x
```
...
> Henning Teickner (`r format(Sys.Date(), "%Y")`). _irpeat: Simple Functions to Analyse Mid Infrared Spectra of Peat Samples'_. Accessed `r format(Sys.Date(), "%d %b %Y")`. Online at <https://github.com/henningte/irpeat>.
